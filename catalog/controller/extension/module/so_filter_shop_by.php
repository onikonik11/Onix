	...
	public function filter_data(){
		...
		if($product_data != "" && count($product_data) > 0){
			...
			foreach ($product_data as $result) {
				...
				$images_2nd = $this->model_catalog_product->getProductImages($result['product_id']);
				if(isset($images_2nd[0]['image']) && !empty($images_2nd[0]['image'])) $images_2nd = $images_2nd[0]['image'];
				else $images_2nd = false;	
								
				$data['products'][] = array(
					'discount'  => $discount,
					'thumb2' 	=> $this->model_tool_image->resize($images_2nd, $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_width'), $this->config->get('theme_' .$this->config->get('config_theme') . '_image_product_height')),
		...
		if(isset($_COOKIE["listingType"]) && $_COOKIE["listingType"]){$listingType = $_COOKIE["listingType"]; }
 		else{$listingType = $this->soconfig->get_settings('product_catalog_mode') ? 'list' : 'grid';}
		...
		$device_res='col-lg-3 col-md-4 col-sm-4 col-xs-6';
		/*$devices = array('lg' => 'Desktops','md' => 'Tablet Landscape','sm' => 'Tablet Portrait','xs' => 'Phones');

		foreach ($devices as $key => $value) {
			$device_res .=  $device_res . ' col-'.$key.'-'.(12 / $this->soconfig->get_settings('product_catalog_column_'.$key)); 
		}
